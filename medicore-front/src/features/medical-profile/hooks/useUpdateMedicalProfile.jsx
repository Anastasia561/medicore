...
export const useUpdateMedicalProfile = ({setGeneralError, setError}) => {
    ...
    return useMutation({
        mutationFn: async (profileData) => {
            const res = await axiosPrivate.put("/patients/medical-profile", profileData);
            if (res.data?.error) {
        ...
        onError: (err) => {
            ...
            if (status === 400) {
                ...
                if (Array.isArray(validationErrors) && validationErrors.length > 0) {
                    ...
                    setGeneralError(data?.error?.message || "Update failed");
                }
            } else if (status === 401) {
                navigate("/login");
            } else if (status === 403) {
